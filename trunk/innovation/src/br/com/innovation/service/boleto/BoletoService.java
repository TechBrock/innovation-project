...
public class BoletoService {
	...
	public static byte[] gerarBoleto(UsuarioVo usuVo, EnderecoVo endVo, Double valorCompra){
		BoletoVo boletoVo = new BoletoVo();
		Calendar dataAtual = Calendar.getInstance();
		boletoVo.setIdCliente(usuVo.getId());
		//boletoVo.setIdCompra(compra.getId());
		boletoVo.setIdCompra(1);
		boletoVo.setValorBoleto(valorCompra);
		...
		Sacado sacado = Sacado.newSacado()  
				.withNome(usuVo.getNome())  
				.withCpf(usuVo.getCpf())  
				.withEndereco(endVo.getLogradouro()+","+endVo.getNumero()+" - "+endVo.getComplemento())  
				.withBairro(endVo.getBairro())  
				.withCep(endVo.getCep())  
				.withCidade(endVo.getNomeCidade())
				.withUf(endVo.getUf());  
